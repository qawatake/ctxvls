...
bench: bin/benchstat
	...
	go test -modfile=go_test.mod -bench=. -benchmem . -count $(BENCH_COUNT) | tee bench1.txt
	perl -pi -e 's|(github.com/qawatake/ctxvls/internal/ctxvls)(\d+)|$$1 . ($$2+1)|ge' ctxvls.go
	go test -modfile=go_test.mod -bench=. -benchmem . -count $(BENCH_COUNT) | tee bench2.txt
	perl -pi -e 's|(github.com/qawatake/ctxvls/internal/ctxvls)(\d+)|$$1 . ($$2+1)|ge' ctxvls.go
	go test -modfile=go_test.mod -bench=. -benchmem . -count $(BENCH_COUNT) | tee bench3.txt
